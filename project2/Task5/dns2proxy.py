...
def requestHandler(address, message):
    ...
    try:
        ...
        try:
			...
			spoof_q= str(q).split(" ")[0][:-1]
			if spoof_q not in spoof:
				save_req(LOGREQFILE,'Client IP: ' + address[0] + '     request is    ' + str(q))
			elif spoof_q in spoof:
				save_req(LOGREQFILE,'Client IP: ' + address[0] + '     request is    ' + str(q) + '  ' + spoof[spoof_q])
